    ...
    ui(new Ui::MainWindow)
{
   ...
   // connect(hueSaturationForm, &huesaturation::parametersAccepted, this, &MainWindow::on_MonochromeParametersChanged);
    ...
    connect(hueSaturationForm, &huesaturation::saturationChanged, this, &MainWindow::saturationChanged);
    connect(hueSaturationForm, &huesaturation::hueChanged, this, &MainWindow::hueChanged);
...
void MainWindow::hueChanged(int hue)
{
    ...
    for (int y = 0; y < monochromeImage.height(); ++y) {
        for (int x = 0; x < monochromeImage.width(); ++x) {
            ...
            QColor newColor = QColor::fromHsv((hue * 359) / 360, originalColor.saturation(), gray);
...
void MainWindow::saturationChanged(int saturation)
{
    ...
    for (int y = 0; y < monochromeImage.height(); ++y) {
        for (int x = 0; x < monochromeImage.width(); ++x) {
            QColor originalColor = monochromeImage.pixelColor(x, y);
            int gray = qGray(originalColor.rgb());
            QColor newColor = QColor::fromHsv(originalColor.hue(), (saturation+100) * 255 / 200, gray);
            monochromeImage.setPixelColor(x, y, newColor);
        }
    }
    CopyColorImage = monochromeImage.copy();
